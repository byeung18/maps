...
typedef struct IntersectionProperties {
    ...
    IntersectionProperties(const Node& n) : osmNodeID(n.getID()), pos {
        n.getLat(), n.getLon()
    }
    {
    }
...
public:
    typedef adjacency_list<vecS, vecS, bidirectionalS, IntersectionProperties,
    StreetSegmentProperties,
    no_property,
    vecS> Graph;
...
public:
    ...
    unsigned long long getNumberOfIntersections() const {
        return num_vertices(G);
    }

    unsigned long long getNumberOfStreetSegments() const {
        return num_edges(G);
    }

    unsigned long long getNumberOfStreets() const {
        return streetProperties.size();
    }
    ...
    unsigned long long getNumberOfPointsOfInterest() const {
        return pointsOfInterest.size();
    }

    const POIProperties& getPointOfInterest(unsigned i) const {
    ...
    const StreetProperties& getStreet(unsigned i) const {
        return streetProperties[i];
    }

    const StreetSegmentProperties& getStreetSegment(unsigned i) const {
        return G[streetSegmentDescriptors[i]];
    }
    ...
    LatLon getFeaturePoint(unsigned featureID, unsigned pointIdx) const {
        return features[featureID].coords[pointIdx];
    }

    unsigned getFeaturePointCount(unsigned featureID) const {
        return features[featureID].coords.size();
    }
    ...
    double getStreetSegmentSpeedLimit(unsigned i) const {
        return G[streetSegmentDescriptors[i]].speedLimit;
    }
    ...
    LatLon getIntersectionPosition(unsigned intersectionID) const {
        return G[intersectionID].pos;
    }
};
