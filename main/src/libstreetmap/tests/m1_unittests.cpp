...
SUITE(M1_Public_Functionality_Tests) {
	...
	// This is the unit test that we added. It takes the intersection ID from the previous      	             
	// test and it takes the test cases from the following test.    	
	TEST_FIXTURE(Map_Fixture, intersection_name_street_name_ID_functionality) {
		...
		result = find_intersection_street_names(1376);
		std::sort(result.begin(), result.end());
		CHECK_EQUAL(expected_result, result);
